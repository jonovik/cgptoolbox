...
def setup(app):
    ...
    app.add_config_value('ggplot_pre_code', 'from rnumpy import *\nr.library("ggplot2")', True)
...
def render_figures(code, code_path, output_dir, output_base, context,
    ...
    *output_base*
    ...
    for i, code_piece in enumerate(code_pieces):
        ...
        images = []
        fig_managers = [None]  # _pylab_helpers.Gcf.get_all_fig_managers()
...
def run(arguments, content, options, state_machine, state, lineno):
    ...
    if len(arguments):
        if not config.ggplot_basedir:
            ...
            source_file_name = os.path.join(setup.confdir, config.ggplot_basedir,
                                            directives.uri(arguments[0]))
    ...
    for j, (code_piece, images) in enumerate(results):
        ...
        result = format_template(
            ...
            html_show_formats=config.ggplot_html_show_formats,
            caption=caption)
