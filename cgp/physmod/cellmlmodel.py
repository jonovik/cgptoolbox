...
class Cellmlmodel(Namedcvodeint):
    ...
    def __init__(self,  # pylint: disable=W0102,E1002,R
        ...
        **kwargs):
        ...
        """
        L = exposure_workspace.split("/", 1)
        if len(L) == 1:
            workspace = L[0]
            exposure = get_latest_exposure(
                "http://models.cellml.org/workspace/" + workspace)
        else:
            exposure, workspace = L
...
def get_all_workspaces(get_latest_exposures=False,
    ...
    if get_latest_exposures:
        w["exposure"] = [get_latest_exposure(i) for i in w["uri"]]
        w["ew"] = ["/".join(i) for i in zip(w["exposure"], w["workspace"])]
    return dict2rec(w).view(np.recarray)

def get_latest_exposure(uri):
    """
    Get latest exposure from models.cellml.org.
    
    :param str uri: The web address of a cellml.org workspace, as returned by 
        :func:`get_all_workspaces`.
    
    >>> uri = "http://models.cellml.org/workspace/modularmassactionprimer"
    >>> get_latest_exposure(uri)
    '29a0ec2468a49a64a123f927083260f0'
    """
    latest_exposure = ('<a class="button" href="http://models\.cellml\.org/' +
        'exposure/(\w*?)">Latest Exposure</a>')
    with closing(urlopen(uri)) as f:
        match = re.search(latest_exposure, f.read())
    if match:
