...
def get_all_workspaces(
    ...
    with closing(urlopen(url)) as f:
        d = json.load(f)
    title, uri = zip(*d["values"])
    workspace = [i.split("/")[-1] for i in uri]
    exposure = []
    for i in uri:
        with closing(urlopen(i)) as f:
            match = re.search(latest_exposure, f.read())
        exposure.append(match.group(1) if match else "")
    ew = [(e + "/" + w) for e, w in zip(exposure, workspace)]
    w = np.rec.fromarrays([title, uri, exposure, workspace, ew], 
        names="title uri exposure workspace ew".split())
    return w
