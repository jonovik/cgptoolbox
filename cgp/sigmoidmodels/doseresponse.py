...
def Hill(x,theta,p):
    """
    The Hill function (Hill, 1910) gives a sigmoid increasing dose-response function 
    crossing 0.5 at the threshold theta with a steepness determined by the 
    Hill coefficient p.
    ...
    * Hill AV (1910) 
      `The possible effects of the aggregation of the molecules of haemoglobin on its dissociation curves <http://jp.physoc.org/content/40/supplement/i.full.pdf+html>`_ 
...
def nonmono(x,mu,sigma):
    '''
    Bell-shaped gene regulation function with max-value 1, in form of a scaled probability density
    function for the normal distribution. Used as the non-monotonic gene regulation function in Gjuvsland et al. (2011).
    ...
    * Gjuvsland AB, Vik JO, Wooliams JA, Omholt SW (2011) 
      `Order-preserving principles underlying genotype-phenotype maps ensure high additive proportions of genetic variance <http://onlinelibrary.wiley.com/doi/10.1111/j.1420-9101.2011.02358.x/full>`_
      Journal of Evolutionary Biology 24(10):2269-2279.
    '''
...
def R_logic(Z1,Z2,index,name=False):
    """
    This function outputs the continuous equivalent of one of 16 boolean functions of Z1 and Z2.
    
    :param Z1: scalar in [0,1]
    :param Z2: scalar in [0,1]
    :param index: integer between 1 and 16 indicating Boolean function (see table below)
    :param names: Boolean wheter to return function value (default) or function name (names=True)
    :return: function value in the range [0,1].
    ...
    Example - Create boolean truth tables for all 16 boolean functions::
      >>> X1=[0,0,1,1]
      >>> X2=[0,1,0,1]
      >>> [(R_logic([],[],ind,name=True),[R_logic(x1,x2,ind) for x1,x2 in zip(X1,X2)]) for ind in range(1,17)] #doctest: +NORMALIZE_WHITESPACE
      [('AND(X1,X2)', [0, 0, 0, 1]),                                                                                           
