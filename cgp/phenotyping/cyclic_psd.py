...
Functionality
1. Use module cellmodel for initializing ODEs from the cellml specification 
...
class Cyclic(Cellmlmodel):
    ...
    def cycle(self, y=None, ignore_flags=False):
        ...
        t, Y, _flag = zip(*result)  # pylint: disable=W0142
    ...
    def next_extremum(self, y=None, ignore_flags=False):
        ...
        if (Y.view(float)[:, self.cycle_state].max() 
            == Y[-1][0][self.cycle_state]):
            stats = 'peak'
        else:
            stats = 'bottom'
    ...
    def limit_cycle(self, parameters=None, timecourse=False): #pylint:disable=R
        ...
        try:
            ...
            for tstop in [2**N for N in range(6,32)]:
                ...
                if stats == 'peak': #find first bottom point for MB
                    ...
                    raise Exception('Extremal point was unexpectedly minimum')
                ...
                ext1 = np.array([cycle_extrema(t1 - t1[0], Y1, var) 
                    for var in Y1.dtype.names])
                ext2 = np.array([cycle_extrema(t2 - t2[0], Y2, var) 
                    for var in Y2.dtype.names])
                ext1[0, 3] = 0.0
                ext2[0, 3] = 0.0
                diffmax = (np.max(np.abs(ext1[:, 0] - ext2[:, 0])
                    / ((ext1[:, 0] + ext2[:, 0])/2)))
                diffmin = (np.max(np.abs(ext1[:, 2] - ext2[:, 2])
                    / ((ext1[:, 2] + ext2[:, 2])/2)))
                diffmaxt = (np.max(np.abs(ext1[:, 1] - ext2[:, 1]))
                    / np.mean([p1, p2]))
                diffmint = (np.max(np.abs(ext1[:, 3] - ext2[:, 3]))
                    / np.mean([p1, p2]))
                solvelog.info("time %07d per %f maxt %f mint %f max %f min %f" 
                    % (tint[-1], diff2, diffmaxt, diffmint, diffmax, diffmin))
                
                maxdiff = 1e-4
                if (diff2 < maxdiff and diffmaxt < maxdiff 
                    and diffmint < maxdiff and diffmax < maxdiff 
                    and diffmin < maxdiff):
                    convergence = True
                    break 
        ...
        extrema = [(var, cycle_extrema(t - t[0], Y, var)) 
            for var in Y.dtype.names]
        
        for var in extrema:
            # ttb = time from MP bottom to var[0].bottom
            # ttp = time from var[0].bottom to var[0].peak
            for i in "bottom peak ttb ttp".split():
                phenotype_names.append(var[0] + '_' + i)
            phenotype_values.extend([var[1][i] for i in 2, 0, 3])
    ...
    def steady_state(self, parameters=None, timecourse=False):#pylint:disable=R
        """
        ...
        try:
            for tstop in [4**N for N in range(3, 16)]:
                ...
                t, Y, _flag = self.integrate(t=[max(self.t) + tstop])
                diff1 = (Y.view(float).max(axis=0) 
                    - Y.view(float).min(axis=0)).max()
                msg = "cGP_leloup.py: Max state difference: %s"
                solvelog.debug(msg % diff1)
                if diff1 < 1e-8:   
                    convergence = True
                    break
            if not convergence:
        ...
        phenotype_names = ['period', 'tconv']
        phenotype_values = [np.inf, tstop]
        
        extrema = [(var, Y[var][0], Y[var][0], np.inf, np.inf) 
            for var in Y.dtype.names]
        ...
        for var in extrema:
            ...
            for i in "bottom peak ttb ttp".split():
                phenotype_names.append(var[0] + '_' + i)
            phenotype_values.extend(var[1:5])
    ...
    def __init__(self, exposure_workspace=
        ...
        self.cycle_state = cycle_state  # index of rootfinding state variable
...
class Leloup(Cyclic):
    ...
    def __init__(self, exposure_workspace="975e54d2d30fc68ebe488375a0731cb2/"
        "leloup_goldbeter_2004", **kwargs):
        """
...
def interpolate(t, Y, N_interp):
    """Interpolate to N_interp linearly spaced time-points."""
    import scipy
